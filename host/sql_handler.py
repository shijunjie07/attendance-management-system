...
class SQLHandler:
    ...
    def get_classes(self, by:str, **query_params):
        ...
        Args:
            by (str): student, lecturer, date, location, location_date, student_date
        ...
        elif by == 'location_date':
            location = query_params.get('location')
            date = query_params.get('date')
            if location and date:
                start_of_day = datetime.strptime(date, '%Y-%m-%d')
                end_of_day = start_of_day + timedelta(days=1)
                query = query.filter_by(location=location).filter(
                    Class.start_time.between(start_of_day, end_of_day))
    
        elif by == 'student_date':
            student_id = query_params.get('student_id')
            date = query_params.get('date')
            if student_id and date:
                start_of_day = datetime.strptime(date, '%Y-%m-%d')
                end_of_day = start_of_day + timedelta(days=1)
                query = query.filter_by(student_id=student_id).filter(
                    Class.start_time.between(start_of_day, end_of_day))
